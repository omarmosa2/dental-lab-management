...
export class PhoneValidator {
  ...
  private static readonly SYRIAN_PATTERNS = {
    mobile: /^(?:\+?963|0)?9[0-9]{8}$/,
    landline: /^(?:\+?963|0)?[1-9][0-9]{6,7}$/,
  };

  // International format patterns
  private static readonly INTERNATIONAL_PATTERN = /^\+?[1-9]\d{1,14}$/;

  /**
   * Validate and format a phone number
   */
  static validate(phone: string, _defaultCountryCode = '963'): PhoneValidationResult {
   ...
   */
  private static validateSyrian(cleaned: string): PhoneValidationResult {
   ...
   */
  private static formatSyrian(cleaned: string): string {
   ...
   */
  private static validateInternational(cleaned: string): PhoneValidationResult {
    // Remove + if present
   ...
   */
  static isValidForWhatsApp(phone: string): boolean {
    const result = this.validate(phone);
