...
export class OrderRepository {
  ...
  generateOrderNumber(): string {
    ...
    const results = executeQuery<{ count: number }>(
      'SELECT COUNT(*) as count FROM orders WHERE date_received >= ? AND date_received < ?',
      [startOfDay, endOfDay]
    );
   ...
   */
  findAll(filters?: OrderFilters, pagination?: PaginationParams): Order[] {
    ...
    return executeQuery<Order>(sql, params);
  }
  ...
  findById(id: number): Order | null {
    const results = executeQuery<Order>(
  ...
  findByOrderNumber(orderNumber: string): Order | null {
    const results = executeQuery<Order>(
      'SELECT * FROM orders WHERE order_number = ? AND deleted_at IS NULL',
      [orderNumber]
    );
    return results[0] || null;
  }
  ...
  update(dto: UpdateOrderDto): Order {
    ...
    if (!existing) {
      throw new NotFoundError('Order', dto.id);
    }
  ...
  permanentDelete(id: number): void {
    executeNonQuery('DELETE FROM orders WHERE id = ?', [id]);
  }

  /**
   * Get orders count (excluding soft-deleted)
   */
  count(filters?: OrderFilters): number {
