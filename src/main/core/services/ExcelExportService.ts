...
import type { Order, Dentist, Material, Expense, Payment, Worker } from '../../../shared/types/api.types';
...
export class ExcelExportService {
   ...
   */
  private validateExportData(data: any[], entityName: string): void {
    ...
    log.info(`ExcelExportService: Validation passed for ${entityName}`, { count: data.length });
  }
  ...
  private validateExportDirectory(dirPath: string): void {
    try {
      ...
      log.info('ExcelExportService: Export directory validated', { dirPath });
    } catch (error) {
      log.error('ExcelExportService: Export directory validation failed', error);
   ...
   */
  async exportDentists(dentists: Dentist[]): Promise<string> {
    try {
      log.info('ExcelExportService: Exporting dentists', { count: dentists.length });
      ...
      log.info(`ExcelExportService: Dentists exported successfully`, { 
        filepath, 
      ...
      log.error('ExcelExportService: Failed to export dentists', error);
   ...
   */
  async exportOrders(orders: Order[]): Promise<string> {
    try {
      log.info('ExcelExportService: Exporting orders', { count: orders.length });
      ...
      orders.forEach((order, index) => {
        const toothNumbers = typeof order.tooth_numbers === 'string'
          ? order.tooth_numbers
          : JSON.parse(order.tooth_numbers as any).join(', ');
      ...
      this.sendProgress(100, 'تم التصدير بنجاح');
      log.info(`ExcelExportService: Orders exported successfully`, { 
        filepath, 
        count: orders.length,
        size: stats.size 
      });
      ...
      this.sendProgress(0, 'فشل التصدير');
      log.error('ExcelExportService: Failed to export orders', error);
   ...
   */
  async exportMaterials(materials: Material[]): Promise<string> {
    try {
      log.info('ExcelExportService: Exporting materials', { count: materials.length });
      ...
      this.sendProgress(100, 'تم التصدير بنجاح');
      log.info(`ExcelExportService: Materials exported successfully`, { 
        filepath, 
      ...
      this.sendProgress(0, 'فشل التصدير');
      log.error('ExcelExportService: Failed to export materials', error);
   ...
   */
  async exportExpenses(expenses: Expense[]): Promise<string> {
    try {
      log.info('ExcelExportService: Exporting expenses', { count: expenses.length });
      ...
      this.sendProgress(100, 'تم التصدير بنجاح');
      log.info(`ExcelExportService: Expenses exported successfully`, { 
        filepath, 
      ...
      this.sendProgress(0, 'فشل التصدير');
      log.error('ExcelExportService: Failed to export expenses', error);
   ...
   */
  async exportPayments(payments: Payment[]): Promise<string> {
    try {
      log.info('ExcelExportService: Exporting payments', { count: payments.length });
      ...
      this.sendProgress(100, 'تم التصدير بنجاح');
      log.info(`ExcelExportService: Payments exported successfully`, { 
        filepath, 
      ...
      this.sendProgress(0, 'فشل التصدير');
      log.error('ExcelExportService: Failed to export payments', error);
   ...
   */
  async exportWorkers(workers: Worker[]): Promise<string> {
    try {
      log.info('ExcelExportService: Exporting workers', { count: workers.length });
      ...
      this.sendProgress(100, 'تم التصدير بنجاح');
      log.info(`ExcelExportService: Workers exported successfully`, { 
        filepath, 
      ...
      this.sendProgress(0, 'فشل التصدير');
      log.error('ExcelExportService: Failed to export workers', error);
